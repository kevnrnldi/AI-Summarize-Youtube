...
export async function summarizeVideo(req, res) {
  try {
    ...
    console.log("Video Berhasil disimpan dengan Id:", video._id);

    const transcriptSave = await Transcript.create({
      youtubeId: videoId,
      text: transcript,
    });

    console.log("transcript Berhasil disimpan dengan Id:", transcriptSave._id);
